...
export default function Page(props: Props) {
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        try {
            const resp = await signIn("register", { ...values, redirect: false });
        ...
        <div className="w-full flex flex-col items-center pt-10">
            ...
            <Form { ...form }>
                <form onSubmit={ form.handleSubmit(onSubmit) } className="w-1/4 space-y-8">
                    ...
                    <FormDescription>Already have an account? <Link href={{ pathname: "/auth/login", query: { callbackUrl } }} className="text-blue-600 underline underline-offset-2">Log in</Link></FormDescription>
                </form>
            </Form>
        </div>
    );
